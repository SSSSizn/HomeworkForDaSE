...
int fork(void)
{
  ...
  np->parent = curproc;
  *np->tf = *curproc->tf;
  // Set thr tickets for the new proc to the ones for the current proc
...
{  
  ...
  {  
    ...
    // Total tickets of all runnable processes  
    long total = 0;  
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)  
    {  
      if (p->state == RUNNABLE)  
    ...
    // Generate a winner ticket at random
    long win_ticket = random_at_most(total);  
    long cur_total = 0;  
    // Iterate through the process table to find a runnable process  
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)    
  {    
    // Skip processes that are not in the RUNNABLE state  
    if (p->state != RUNNABLE)    
      continue;    
     
    cur_total += p->ticket;        
    // Check if the current total tickets have exceeded the winning ticket threshold  
